...
function InnerCore() {
  ...
  return (
    <Sphere ref={mesh} args={[0.55, 32, 32]}>
      <meshStandardMaterial color="#7c6dff" emissive="#7c6dff" emissiveIntensity={0.7} roughness={0.2} metalness={0.8} />
    </Sphere>
  )
}
...
function WireShell() {
  ...
  return (
    <Icosahedron ref={mesh} args={[1.45, 2]}>
      <meshBasicMaterial color="#14d9ff" wireframe transparent opacity={0.12} />
    </Icosahedron>
  )
}
...
export function HeroCanvas() {
  return (
    <Canvas camera={{ position: [0, 0, 5], fov: 45 }} dpr={[1, 2]} gl={{ antialias: true, alpha: true }}>
      ...
      <pointLight position={[5, 5, 5]} intensity={2.5} color="#14d9ff" />
      <pointLight position={[-5, -3, 2]} intensity={1.8} color="#7c6dff" />
      <pointLight position={[0, 3, -4]} intensity={1.2} color="#3b82f6" />
      <pointLight position={[0, 0, 3]} intensity={0.6} color="#14d9ff" />
      <Suspense fallback={null}>
        ...
        <OrbitDot radius={1.7} tilt={1.2} speed={0.6} color="#14d9ff" offset={3.14} />
        <OrbitDot radius={2.1} tilt={-0.6} speed={-0.5} color="#3b82f6" offset={2} />
        <OrbitDot radius={2.1} tilt={-0.6} speed={-0.5} color="#3b82f6" offset={5} />
        <OrbitDot radius={2.5} tilt={0.3} speed={0.4} color="#7c6dff" offset={4} />
        <OrbitDot radius={2.5} tilt={0.3} speed={0.4} color="#7c6dff" offset={1.5} />
      </Suspense>
    </Canvas>
  )
}
